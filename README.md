...
Реализован **рабочий MVP** семейного трекера расходов: backend (Kotlin + Spring Boot), frontend (Vue 3 + TypeScript), PostgreSQL, Redis, Docker. Пользователи регистрируются, создают общие кошельки, ведут доходы и расходы по категориям, делят чеки, задают бюджет на месяц и настраивают автоплатежи.
